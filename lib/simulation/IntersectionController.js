...
export class IntersectionController {
  ...
  canEnter(veh, lane, allVehicles) {
    if (lane.freeFlow) {
      // Free-flow: proceed unless a yielded stream is crossing
      return !this._mustYield(lane, allVehicles);
    }

    // Controlled: must have green signal and clear box
    if (!this.light.isGreen(lane.id)) return false;
  ...
  getCrossLeader(veh, lane, allVehicles) {
    ...
    for (const v of allVehicles) {
      ...
      // Is v ahead of veh on the path? Use path index as proxy.
      if (v.pathIdx <= veh.pathIdx) continue;
