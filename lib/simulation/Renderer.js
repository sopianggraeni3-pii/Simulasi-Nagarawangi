...
export class Renderer {
   ...
   */
  draw(vehicles, lightSnap, metrics) {
    const { ctx } = this;
    ...
    this._drawHUD(lightSnap, metrics, vehicles.length);
  }
  ...
  _drawHUD(ls, metrics, vCount) {
    ...
    const timerText   = `${Math.ceil(ls.timer)}s / ${ls.totalGreen}s`;
