...
const SPAWN_GAP       = 8;    // m — minimum clearance at spawn point
...
export class SimulationEngine {
  ...
  _doApproach(veh, lane, dt) {
    ...
    // Transition at stop line
    if (stopDist < STOP_THRESHOLD) {
      if (canEnter && this.controller.canEnter(veh, lane, this.vehicles)) {
  ...
  _doQueued(veh, lane, dt) {
    ...
    // Maintain minimum gap to vehicle ahead (gentle compaction)
    const { gap: lGap } = lane.getLeader(veh);
    const minQ = veh.type.minGap + veh.type.length * 0.5;
    if (lGap < minQ && lGap > 0) {
      const pullback = (minQ - lGap) * 0.25;
      const dx = lane.stopLine.x - veh.x;
      const dy = lane.stopLine.y - veh.y;
      const d  = Math.hypot(dx, dy) || 1;
      veh.x -= (dx / d) * pullback * dt;
      veh.y -= (dy / d) * pullback * dt;
    }

    // Try to release once signal changes and we are at the front
    const canEnter = this.controller.canEnter(veh, lane, this.vehicles);
  ...
  _enterIntersection(veh, lane) {
    veh.state   = 'cross';
    veh.pathIdx = 0;
    // Snap to first cross waypoint (stop line position)
    veh.x       = lane.crossPath[0].x;
    veh.y       = lane.crossPath[0].y;
    veh.speed   = Math.max(veh.speed, 1.5); // at least creep speed
  }
